...
// ==/UserScript==
/* globals parseDOM createApp unsafeWindow GM_addStyle tempVue reactive */
...
const extendDefaultScheme = (newScheme) =>
...
const defaultSchemeWithPrivateFilter = extendDefaultScheme({
    privateFilter: [
...
function schemeParser(scheme) {
    ...
    const parseElement = ({ type, model, innerText, label, labelBefore, callback, ...rest }) => {
  ...
  <div class="fixed bottom-0 right-0 z-9999 rounded bg-zinc-800 max-w-full p-2 m-2"
    ...
    constructor(state, stateLocale, scheme=defaultScheme) {
        ...
        this.app = createApp({
            ...
            template: this.template(parsedScheme)
        });
        this.app.mount("#tapermonkey-app");

        // put vue prop back to avoid collisions
