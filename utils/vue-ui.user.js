// ==UserScript==
// @name         menu-ui
// @description  creates menu ui with vue and tailwind
...
class VueUI {
  ...
  <div class="fixed bottom-0 right-0 z-9999 rounded bg-zinc-800 max-w-full p-2 m-2"
    ...
    <template v-if="!state.hidden">
      ...
      <div v-if="stateLocale.filterOptions.PRIVACY_FILTER" class="flex items-center bg-zinc-900 py-2 px-2 m-1 font-mono rounded">
        ...
        <button v-if="checkAccess" type="button" @click="checkAccess"
      ...
      <!-- DURATION FILTER -->
      <div v-if="stateLocale.filterOptions.DURATION_FILTER" class="flex items-center bg-zinc-900 py-1 px-2 m-1 font-mono rounded">
    ...
    constructor(state, stateLocale) {
        ...
        this.vue = createApp({
            setup: () => {
                const opts = { state, stateLocale };
                if (stateLocale.filterOptions.PRIVACY_FILTER) {
                    Object.assign(opts, { checkAccess: unsafeWindow.checkAccess });
                }
                return opts;
            },
